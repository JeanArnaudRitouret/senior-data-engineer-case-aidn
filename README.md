...
Currently, the only consent-related artefact on disk is the `raw.patient_consents` CDC event log and its staging projection `staging.stg_patient_consents`.
...
|---|---|---|---|---|---|
| Ingest | `raw.*` | dlt-managed: merge per table (all four via pg_replication) | Per-table merge key (pg_replication internal `dedup_sort` on `lsn asc`): `appointments` on `event_id`; `providers` on `provider_id`; `patients` and `patient_consents` on `lsn` | No | Yes (except `patients.name` — never ingested) |
...
- **Idempotent re-runs by disposition:** all four CDC tables use `write_disposition="merge"` via `pg_replication`; re-processing the same WAL events produces no new rows because pg_replication tracks `confirmed_flush_lsn` per slot and the per-table merge key collapses duplicates.
  - `appointments` — merge on `event_id`
  - `providers` — merge on `provider_id`
  - `patients` and `patient_consents` — merge on `lsn` (one row per WAL event)
